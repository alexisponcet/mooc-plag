...
import jplag.CustomComparatorReverse;
...
public class Program implements ProgramI {
	...
	private static final String FILENAME_DATA = "_data";

	private static final String FILENAME_ERR = "_errorsNumber";

	private static final String DIRECTORYNAME_SERIALIZABLE = "serializable";

	private String assignment;

	private int numberOfPreviousSubmissions;

	private HashMap<String, Integer> restartStudent = new HashMap<String, Integer>();

	private HashMap<String, Integer> majDirectory = new HashMap<String, Integer>();

	public Program(Options options) throws jplag.ExitException {
		this.options = options;
		assignment = this.options.root_dir;
		if ((assignment.substring(assignment.length() - 1)).equals("/"))
			assignment = assignment.substring(0, assignment.length() - 1);
		this.options.root_dir = assignment + "/submissions/";
	...
	private void compare() throws jplag.ExitException {
		...
		for (i = size - 1; i >= numberOfPreviousSubmissions; i--) {
			s1 = submissions.elementAt(i);
			if (s1.struct == null) {
				count += (size - i - 1);
				continue;
	...
	private void createSubmissions() throws jplag.ExitException {
			        ...
			        if (fileName.contains(" ")){
			    		try { 
			    			if (fileName.substring(0, 1).equals("~")){
			    				Integer.parseInt(fileName.substring(1, fileName.indexOf(" "))); 
			    				restartStudent.put(fileName.substring(1, fileName.length()), 0);
			    				majDirectory.put(fileName.substring(1, fileName.length()), 0);
			    			} else {
			    				Integer.parseInt(fileName.substring(0, fileName.indexOf(" "))); 
			    				majDirectory.put(fileName.substring(0, fileName.length()), 0);
			    			}
							return true;
						} catch (Exception e) { 
								return false;
		...
		CustomComparatorReverse comparator = new CustomComparatorReverse();
		...
		for (int i = 0; i < list.length; i++) {
			...
			if (!subm_dir.isDirectory()) {
				...
				submissions.addElement(new Submission(name, f, this, get_language(), get_suffixes()));
				continue;
			}
			...
			if (file_dir.isDirectory()) {
				if (options.basecode.equals(subm_dir.getName())) {
					basecodeSubmission = new Submission(subm_dir.getName(), file_dir, options.read_subdirs, this, get_language(), get_suffixes());
				} else {
					submissions.addElement(new Submission(subm_dir.getName(), file_dir, options.read_subdirs, this, get_language(), get_suffixes())); // -s
				}
			} else
	...
	private void createSubmissionsExp() throws jplag.ExitException {
		...
		for (int i = 0; i < list.length; i++) {
			...
			if (file_dir != null && file_dir.isDirectory())
				submissions.addElement(new Submission(subm_dir.getName(), file_dir, options.read_subdirs, this, this.get_language(), get_suffixes())); // -s
	...
	private void parseAll() throws jplag.ExitException {
		...
		File dir = new File(assignment + "/" + DIRECTORYNAME_SERIALIZABLE + "/");
		if (!dir.exists()) {
		    try{
		        dir.mkdir();
		    } 
		    catch(SecurityException se){
		      	print("\n" + "Unable to create the serializable folder !!" + "\n", null);
		    }
		}
		String filename_data = assignment + "/" + DIRECTORYNAME_SERIALIZABLE + "/" + assignment + FILENAME_DATA +".ser";
		String filename_err = assignment + "/" + DIRECTORYNAME_SERIALIZABLE + "/" + assignment + FILENAME_ERR +".ser";
		File file_data = new File(filename_data);
		File file_err = new File(filename_data);
		Vector<Submission> read_ex_submissions = new Vector<Submission>();
		Vector<Submission> ex_submissions = new Vector<Submission>();
		numberOfPreviousSubmissions = 0;
		int sumOfErrors[] = new int[2];
		FileInputStream fin;
		ObjectInputStream ois;

		if (file_data.exists()) {
			try {
				fin = new FileInputStream(filename_data);
				ois = new ObjectInputStream(fin);
				read_ex_submissions = (Vector<Submission>) ois.readObject();
				System.out.println("test");
				for (Submission s : read_ex_submissions){
					if (s.name.substring(0, 1).equals("~"))
						if (!restartStudent.containsKey(s.name.substring(1, s.name.length()).equals("~")))
							s.name = s.name.substring(1, s.name.length());
					if (!restartStudent.containsKey(s.name) && majDirectory.containsKey(s.name))
						ex_submissions.add(s);
				}
				ois.close();
				numberOfPreviousSubmissions = ex_submissions.size();
			}
			catch (Exception e) { 
				print("\n" + "Corrupted data file !!" + "\n", null);
				e.printStackTrace(); 
		...
		if (file_err.exists()) {
			try {
				fin = new FileInputStream(filename_err);
				ois = new ObjectInputStream(fin);
				sumOfErrors = (int[]) ois.readObject();
				ois.close();
				errors = sumOfErrors[0];
				invalid = sumOfErrors[1];
			}
			catch (Exception e) { 
				print("\n" + "Corrupted errors file !!" + "\n", null);
				e.printStackTrace(); 
			}
		} 
		...
		// error (deleted files)
		if (numberOfPreviousSubmissions > totalcount) 
			numberOfPreviousSubmissions = 0;

		int i = numberOfPreviousSubmissions + errors + invalid;
		while (i < submissions.size()) {
			ex_submissions.add(submissions.get(i));
			boolean ok = true;
			boolean removed = false;
			Submission subm = ex_submissions.lastElement();
		...
		submissions = ex_submissions;
		if (count < i) count = i - numberOfPreviousSubmissions;
		
		try {
	      FileOutputStream fout = new FileOutputStream(filename_data);
	      ObjectOutputStream oos = new ObjectOutputStream(fout);
	      oos.writeObject(submissions);
	      oos.close();

	      fout = new FileOutputStream(filename_err);
	      oos = new ObjectOutputStream(fout);
	      sumOfErrors[0] = errors;
	      sumOfErrors[1]= invalid;
	  	  oos.writeObject(sumOfErrors);
	      oos.close();
	    }
